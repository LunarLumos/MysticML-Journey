...
# Keep the other variables constant at their AVERAGE values (holding them at 1 bedroom /
# 1 year old shifted the curve away from the data points)
X_curve_poly = poly.transform(np.column_stack((X_curve,
                                               np.full_like(X_curve, data['Bedrooms'].mean()),
                                               np.full_like(X_curve, data['Age'].mean()))))
